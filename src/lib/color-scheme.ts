...
export function createColorSchemeScript(dataAttribute: string, storageKey: string): void {
	...
	function setDataAttribute(colorScheme: ColorScheme, document = window.document): void {
	...
	window.__colorScheme = {
		...
		sync(document: Document): void {
			setDataAttribute(cachedColorSchemeState.colorScheme, document);
		},
	};
	...
	document.addEventListener("astro:before-swap", (event) => {
		...
		window.__colorScheme.sync(document);
	});
}
